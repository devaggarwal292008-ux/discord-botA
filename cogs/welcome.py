...
class Welcome(commands.Cog):
    ...
    def _draw_gradient_text(
        self, base_img, pos, text, font,
        left_color, right_color, shadow=2
    ):
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        try:
            ...
            bordered = Image.new(
                "RGBA",
                (avatar_size + 2 * border, avatar_size + 2 * border),
                (0, 0, 0, 0)
            )
            ...
            subtext = (
                f"WELCOME TO {guild.name.upper()} | "
                f"YOU ARE OUR {len(guild.members)}TH MEMBER!"
            )
